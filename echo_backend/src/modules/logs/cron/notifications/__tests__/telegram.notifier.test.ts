...
describe('formatLogLine', () => {
  it('should format a log with the +2h shifted date and a GMT+2 label', () => {
    const result = formatLogLine(mockLog())

    expect(result).toBe('[1] [2026-01-01 12:00:00 GMT+2] [ERROR] - worker.ts > Something broke')
  })
})

describe('buildTelegramMessage', () => {
  it('should return just the header when there are no logs', () => {
    const result = buildTelegramMessage([], 'my-device')

    expect(result).toBe('Logs from device my-device:\n\n\n')
  })

  it('should include a single log line under the character limit', () => {
    const result = buildTelegramMessage([mockLog()], 'my-device')

    expect(result).toBe(
      'Logs from device my-device:\n\n\n[1] [2026-01-01 12:00:00 GMT+2] [ERROR] - worker.ts > Something broke'
    )
  })

  it('should include multiple logs separated by double newlines when all fit', () => {
    const logs = [mockLog({ jobId: 1 }), mockLog({ jobId: 2 })]
    const result = buildTelegramMessage(logs, 'my-device')
  ...
  it('should truncate and append a footer when a log would exceed the Telegram limit', () => {
    ...
    const result = buildTelegramMessage([smallLog, hugeLog], 'my-device')
