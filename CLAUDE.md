...
- The optional Telegram cron (`echo_backend/src/modules/logs/logs.cron.ts`) only registers if `LOGS_CRON_SCHEDULE_REGEX`, `LOGS_CRON_WATCHED_LOGS_CATEGORIES`, `LOGS_CRON_TELEGRAM_CHAT_ID`, and `LOGS_CRON_TELEGRAM_BASE_URL` are all present and valid; it tracks last-checked time in `data/last_logs_check.json`.
